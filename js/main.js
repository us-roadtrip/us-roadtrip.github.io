...
var padding = {t:+50, b:+50, l:+70, r:+50};
...
d3.csv('https://docs.google.com/spreadsheets/d/e/2PACX-1vS0U5fYMmF47RZsUfuW_psDoMnyxo_OjJO3pzHXDnkQ4HPCyn4kZMj_HAZblNFfHZGbHuEvmHWJSZn3/pub?output=csv',
    function(d) {
        ...
        var arc = g.selectAll('.arc')
            .data(pie(activity_nested))
            .enter().append('g')
                .attr('class', 'arc');

        arc.append('path')
            .attr('d', path)
            .attr('fill', function(d) {return parseInt(d.data.key) == -1 ? '#fff' : colors[parseInt(d.data.key)]});       
        
        arc.append("text")
            .attr("transform", function(d) { return "translate(" + label.centroid(d) + ")"; })
            .attr("dy", "0.2em")
            .style('font-size', 9)
            .text(function(d) { return activities[parseInt(d.data.key)];})
